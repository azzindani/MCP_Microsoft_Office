...
@mcp.tool()
def set_heading(file_path: str, paragraph_index: int, level: int, open_after: bool = False) -> dict:
    """Apply Heading 1-6 style to paragraph N. level must be 1-6."""
    return engine.set_heading(file_path, paragraph_index, level, open_after)
...
def add_image(
    ...
    return engine.add_image(file_path, paragraph_index, image_path, width_inches, open_after)
...
def set_page_margins(
    ...
    return engine.set_page_margins(file_path, top, bottom, left, right, open_after)
...
def add_header_footer(
    ...
    return engine.add_header_footer(file_path, text, location, open_after)
